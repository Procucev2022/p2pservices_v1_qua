...
class RFQEntityCoverageTest {
    ...
    void testRFQEntityFullCoverage() {
        ...
        RFQEntity diffId = new RFQEntity(99L, "RFQ-100", "buyer@example.com", "SUCCESS", "Subject", "[{}]", "Bengaluru", "2026-09-30", now, now);
        assertNotEquals(entity, diffId);

        RFQEntity diffNum = new RFQEntity(1L, "RFQ-DIFF", "buyer@example.com", "SUCCESS", "Subject", "[{}]", "Bengaluru", "2026-09-30", now, now);
        assertNotEquals(entity, diffNum);

        RFQEntity diffEmail = new RFQEntity(1L, "RFQ-100", "diff@example.com", "SUCCESS", "Subject", "[{}]", "Bengaluru", "2026-09-30", now, now);
        assertNotEquals(entity, diffEmail);

        RFQEntity diffStatus = new RFQEntity(1L, "RFQ-100", "buyer@example.com", "FAILED", "Subject", "[{}]", "Bengaluru", "2026-09-30", now, now);
        assertNotEquals(entity, diffStatus);

        RFQEntity diffSubj = new RFQEntity(1L, "RFQ-100", "buyer@example.com", "SUCCESS", "Diff Subj", "[{}]", "Bengaluru", "2026-09-30", now, now);
        assertNotEquals(entity, diffSubj);

        RFQEntity diffJson = new RFQEntity(1L, "RFQ-100", "buyer@example.com", "SUCCESS", "Subject", "[\"diff\"]", "Bengaluru", "2026-09-30", now, now);
        assertNotEquals(entity, diffJson);

        RFQEntity diffLoc = new RFQEntity(1L, "RFQ-100", "buyer@example.com", "SUCCESS", "Subject", "[{}]", "Chennai", "2026-09-30", now, now);
        assertNotEquals(entity, diffLoc);

        RFQEntity diffDate = new RFQEntity(1L, "RFQ-100", "buyer@example.com", "SUCCESS", "Subject", "[{}]", "Bengaluru", "2026-11-30", now, now);
