...
public class SelfRegistrationServiceImpl implements SelfRegistrationService {
	...
	public ClientRegistrationStatus selfclientRegistrationData(Organization organization) throws AppException {
		...
		if (organization.getCompanyName() == null || organization.getCompanyName().isEmpty()) {
			throw new AppException(HttpStatus.BAD_REQUEST.value(), "Company name is required for registration", null,
		...
		try {
			...
			if (orgTypeObject == null) {
				throw new AppException(HttpStatus.INTERNAL_SERVER_ERROR.value(),
						"Client organization type not configured. Contact admin.", null, null, LocalDateTime.now());
			}

			// Check if organization already exists
			List<Organization> orgList = orgDao.findByCompanyNameAndOrgType(organization.getCompanyName(),
					orgTypeObject);
			String normalizedPhone = PhoneNumberUtils.normalize(organization.getOrganizationPhonenumber());
			User existingUsers = userDao.findByUsernameAndPhoneAndActive(organization.getEmail(), normalizedPhone,
					true);

			if (existingUsers != null) {
				throw new AppException(HttpStatus.CONFLICT.value(),
						"User with the same email and phone number already exists", null, null, LocalDateTime.now());
			}
			...
			return isNewClient ? ClientRegistrationStatus.NEW_CLIENT : ClientRegistrationStatus.EXISTING_CLIENT; // true
																													// if
																													// new
																													// client,
																													// false
			...
			throw new AppException(HttpStatus.INTERNAL_SERVER_ERROR.value(),
					"Something went wrong while processing the registration", null, null, LocalDateTime.now());
		}
	}
...
//	    }
	...
	public boolean vendorRegistration(Organization organization) throws UnsupportedEncodingException {
		...
		if (organization == null) {
			throw new AppException(HttpStatus.BAD_REQUEST.value(), "Organization payload cannot be null",
					ApplicationConstants.VALIDATION_EXCEPTION, ApplicationConstants.FAILURE, LocalDateTime.now());
		}

		// 1. Validate email & phone
		if (organization.getEmail() == null || organization.getEmail().trim().isEmpty()) {
			throw new AppException(HttpStatus.BAD_REQUEST.value(), "Email cannot be empty",
					ApplicationConstants.VALIDATION_EXCEPTION, ApplicationConstants.FAILURE, LocalDateTime.now());
		}

		if (organization.getOrganizationPhonenumber() == null
				|| organization.getOrganizationPhonenumber().trim().isEmpty()) {
			throw new AppException(HttpStatus.BAD_REQUEST.value(), "Phone number cannot be empty",
					ApplicationConstants.VALIDATION_EXCEPTION, ApplicationConstants.FAILURE, LocalDateTime.now());
		}
...
//	    }
		...
		if (checkUserexistWithPhone(organization.getEmail().trim(), normalizedPhone)) {
			throw new AppException(HttpStatus.CONFLICT.value(), ApplicationConstants.USER_ALREADY_ASSOCIATED_TO_ACCOUNT,
					ApplicationConstants.BUSSINESS_EXCEPTION, ApplicationConstants.FAILURE, LocalDateTime.now());
		}
		...
		try {
			...
			if (orgTypeObject == null || vendorStatus == null || evaluationStatus == null) {
				throw new AppException(HttpStatus.INTERNAL_SERVER_ERROR.value(),
						...
						ApplicationConstants.SYSTEM_EXCEPTION, ApplicationConstants.FAILURE, LocalDateTime.now());
			}
...
//	    }
	...
	public List<User> getUsersByPhoneNumber(String phoneNumber) {
		...
		String withPrefix = normalizedPhone.startsWith("+91") ? normalizedPhone
	...
	public Map<String, Object> selfclientRegistrationDataByApp(Organization organization) {
		...
		User userData = userDao.findByUsernameAndPhoneAndActive(organization.getEmail(),
				organization.getOrganizationPhonenumber(), true);
			...
			for (int i = 1; i <= sheet.getLastRowNum(); i++) { // skip header
				...
				try {
					...
					if (existingUser != null) {
						logger.warn("Row {} skipped: user already exists with email {} and phone {}", i, username,
								phone);
						continue;
					}
