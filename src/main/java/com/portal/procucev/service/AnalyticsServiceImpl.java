...
public class AnalyticsServiceImpl implements AnalyticsService {
            ...
            if ("seller".equalsIgnoreCase(type)) {
                ...
                if (stage == 2) {
                    ...
                    items = jdbcTemplate.queryForList(
                        ...
                        "GROUP BY u.uuid, u.full_name, u.username, u.email, u.phone, o.organization_name, u.is_active " +
                        "HAVING rfqCount = 1 " +
                        "ORDER BY MAX(r.created_ts) DESC",
                        queryPattern, queryPattern, queryPattern, queryPattern
                    );
                }
            }
    ...
    public Map<String, Object> searchCompanies(String query) {
        ...
        try {
            ...
            Map<String, Map<String, Object>> subscriptionPlans = new HashMap<>();
            if (!orgIds.isEmpty()) {
                Set<String> planUuids = new LinkedHashSet<>();
                for (Map<String, Object> org : orgRows) {
                    Object planUuid = org.get("subscription_plan_uuid");
                    if (planUuid != null) {
                        String uuid = planUuid.toString().trim();
                        if (!uuid.isEmpty()) {
                            planUuids.add(uuid);
                        }
                    }
                ...
                if (!planUuids.isEmpty()) {
                    String planSql = "SELECT uuid, plan_name, subscription_price FROM subscription_plan WHERE uuid IN (" + placeholders(planUuids.size()) + ")";
                    List<Map<String, Object>> planRows = jdbcTemplate.queryForList(planSql, planUuids.toArray());
                    for (Map<String, Object> row : planRows) {
                        subscriptionPlans.put(getString(row, "uuid", ""), row);
                    }
                }
            }

            Map<String, List<Map<String, Object>>> accountsByOrg = new HashMap<>();
            Map<String, List<Map<String, Object>>> rfqsByOrg = new HashMap<>();
            Map<String, Map<String, Object>> rfqItemByRfqUuid = new HashMap<>();
            Map<String, Integer> recentRfqByOrg = new HashMap<>();
            Map<String, Integer> prevRfqByOrg = new HashMap<>();
            Map<String, List<Map<String, Object>>> quotesByOrg = new HashMap<>();

            if (!orgIds.isEmpty()) {
                String orgPlaceholders = placeholders(orgIds.size());

                List<Map<String, Object>> allAccounts = jdbcTemplate.queryForList(
                    "SELECT org_uuid, uuid, COALESCE(full_name, username) as name, COALESCE(email, '—') as email, COALESCE(phone, '—') as phone, is_active as isActive, DATE_FORMAT(created_ts, '%d %b %Y') as joinedDate " +
                    "FROM user WHERE org_uuid IN (" + orgPlaceholders + ") ORDER BY created_ts DESC",
                    orgIds.toArray()
                );
                for (Map<String, Object> account : allAccounts) {
                    String orgUuid = getString(account, "org_uuid", "");
                    if (orgUuid.isEmpty()) {
                        continue;
                    }
                    List<Map<String, Object>> orgAccounts = accountsByOrg.computeIfAbsent(orgUuid, ignored -> new ArrayList<>());
                    if (orgAccounts.size() < 10) {
                        orgAccounts.add(account);
                    }
                }

                List<Object> rfqParams = new ArrayList<>(orgIds);
                rfqParams.addAll(orgIds);
                List<Map<String, Object>> allRfqs = jdbcTemplate.queryForList(
                    "SELECT uuid, rfq_id, project_desc, created_ts, quote_count, quotation_received, org_uuid, user " +
                    "FROM rfq_header WHERE org_uuid IN (" + orgPlaceholders + ") OR user IN (" + orgPlaceholders + ") ORDER BY created_ts DESC",
                    rfqParams.toArray()
                );

                Set<String> rfqUuids = new LinkedHashSet<>();
                for (Map<String, Object> rfq : allRfqs) {
                    String rfqUuid = getString(rfq, "uuid", "");
                    String rfqOrg = getString(rfq, "org_uuid", "");
                    String rfqUser = getString(rfq, "user", "");

                    if (orgIds.contains(rfqOrg)) {
                        List<Map<String, Object>> orgRfqs = rfqsByOrg.computeIfAbsent(rfqOrg, ignored -> new ArrayList<>());
                        if (orgRfqs.size() < 15) {
                            orgRfqs.add(rfq);
                            if (!rfqUuid.isEmpty()) rfqUuids.add(rfqUuid);
                        }
                    }
                    if (orgIds.contains(rfqUser) && !rfqUser.equals(rfqOrg)) {
                        List<Map<String, Object>> userRfqs = rfqsByOrg.computeIfAbsent(rfqUser, ignored -> new ArrayList<>());
                        if (userRfqs.size() < 15) {
                            userRfqs.add(rfq);
                            if (!rfqUuid.isEmpty()) rfqUuids.add(rfqUuid);
                        }
                ...
                if (!rfqUuids.isEmpty()) {
                    List<Map<String, Object>> rfqItems = jdbcTemplate.queryForList(
                        "SELECT rfq_uuid, category, totalamount FROM rfq_items WHERE rfq_uuid IN (" + placeholders(rfqUuids.size()) + ")",
                        rfqUuids.toArray()
                    );
                    for (Map<String, Object> item : rfqItems) {
                        String rfqUuid = getString(item, "rfq_uuid", "");
                        if (!rfqUuid.isEmpty() && !rfqItemByRfqUuid.containsKey(rfqUuid)) {
                            rfqItemByRfqUuid.put(rfqUuid, item);
                        }
                    }
                }

                List<Map<String, Object>> recentRows = jdbcTemplate.queryForList(
                    "SELECT key_id, count(*) as cnt FROM (" +
                    "SELECT org_uuid as key_id, uuid as rfq_uuid FROM rfq_header WHERE org_uuid IN (" + orgPlaceholders + ") AND created_ts >= DATE_SUB(NOW(), INTERVAL 30 DAY) " +
                    "UNION " +
                    "SELECT user as key_id, uuid as rfq_uuid FROM rfq_header WHERE user IN (" + orgPlaceholders + ") AND created_ts >= DATE_SUB(NOW(), INTERVAL 30 DAY)" +
                    ") t GROUP BY key_id",
                    rfqParams.toArray()
                );
                for (Map<String, Object> row : recentRows) {
                    recentRfqByOrg.put(getString(row, "key_id", ""), getInt(row, "cnt"));
                }

                List<Map<String, Object>> prevRows = jdbcTemplate.queryForList(
                    "SELECT key_id, count(*) as cnt FROM (" +
                    "SELECT org_uuid as key_id, uuid as rfq_uuid FROM rfq_header WHERE org_uuid IN (" + orgPlaceholders + ") AND created_ts >= DATE_SUB(NOW(), INTERVAL 60 DAY) AND created_ts < DATE_SUB(NOW(), INTERVAL 30 DAY) " +
                    "UNION " +
                    "SELECT user as key_id, uuid as rfq_uuid FROM rfq_header WHERE user IN (" + orgPlaceholders + ") AND created_ts >= DATE_SUB(NOW(), INTERVAL 60 DAY) AND created_ts < DATE_SUB(NOW(), INTERVAL 30 DAY)" +
                    ") t GROUP BY key_id",
                    rfqParams.toArray()
                );
                for (Map<String, Object> row : prevRows) {
                    prevRfqByOrg.put(getString(row, "key_id", ""), getInt(row, "cnt"));
                }

                List<Object> quoteParams = new ArrayList<>(orgIds);
                quoteParams.addAll(orgIds);
                List<Map<String, Object>> allQuotes = jdbcTemplate.queryForList(
                    "SELECT " +
                    ...
                    "WHERE (v.vendor_uuid IN (" + orgPlaceholders + ") OR r.org_uuid IN (" + orgPlaceholders + ")) AND v.quote_submitted_date IS NOT NULL " +
                    "ORDER BY v.quote_submitted_date DESC",
                    quoteParams.toArray()
                );
                for (Map<String, Object> q : allQuotes) {
                    String vendorUuid = getString(q, "vendor_uuid", "");
                    String buyerUuid = getString(q, "org_uuid", "");
                    if (orgIds.contains(vendorUuid)) {
                        List<Map<String, Object>> orgQuotes = quotesByOrg.computeIfAbsent(vendorUuid, ignored -> new ArrayList<>());
                        if (orgQuotes.size() < 10) orgQuotes.add(q);
                    }
                    if (orgIds.contains(buyerUuid) && !buyerUuid.equals(vendorUuid)) {
                        List<Map<String, Object>> orgQuotes = quotesByOrg.computeIfAbsent(buyerUuid, ignored -> new ArrayList<>());
                        if (orgQuotes.size() < 10) orgQuotes.add(q);
                    }
                }
            }
            ...
            for (Map<String, Object> org : orgRows) {
                ...
                for (Map<String, Object> q : rawQuotes) {
                    ...
                    quotes.add(Map.of(
                        ...
                        "rfqId", rfqIdStr,
                        "vendorName", vName,
                        "amount", qAmt > 0 ? String.format("₹%,.0f", qAmt) : "₹0",
