...
public interface GmtRfqVendorDao extends JpaRepository<GmtRfqVendors, String> {
	...
	@Query("SELECT  new GmtRfqVendors(g.id,g.vendor.id,g.vendor.companyName,g.vendor.companyId,COALESCE(g.status, 'Notified'),g.query,g.vendor.otherEmails,g.quoteSubmittedDate) from GmtRfqVendors g where g.rfq =:rfq Order By g.createdTS DESC")
