...
public interface RfqDao extends JpaRepository<Rfq, String>{
	...
	List<String> findRFQIdsNoPrRfqByClient(String user);

	@Query("SELECT r FROM Rfq r WHERE  r.user=:user and r.noPrFlag = true Order By r.createdTS DESC")
	List<Rfq> findNoPrRfqByClient(String user);
	...
	void updateRfqByRfqId(String rfqId);
	...
	List<Rfq> getRfqsByNoPrFlagIsTrue(String fullName);
